...
class SelectionPolicyAgent(DQN):
    ...
    def _create_models(self):
        ...
        self.policy_net = (
            MLP(self.state_length, self.action_size, self.hidden_size).to(self.device).double()
        )
        self.target_net = (
            MLP(self.state_length, self.action_size, self.hidden_size).to(self.device).double()
        )
        ...
        for i, x in enumerate(range(20)):
            ...
            while not done:
                with torch.no_grad():
                    ...
                    action = np.argmax(q_values.cpu().numpy())
                state, reward, done, _ = env.step(self.action_to_map(action))
    ...
    def handleTraining(self):
        if len(self.memory) >= self.config.rl.min_memory and (
            ...
            self.avg_grad_value = torch.mean(
                torch.stack([torch.mean(abs(param.grad)) for param in self.policy_net.parameters()])
            )
...
def main(config):
    ...
    logger = Experiment(
        project_name=config.rl.comet.project,
